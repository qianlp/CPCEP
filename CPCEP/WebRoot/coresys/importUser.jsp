...
<script language="JavaScript" type="text/javascript">
	function goUploadFile( obj ){
		...
		if (selectPath_File[selectPath_File.length-1].length>12){
			mini.alert("文件名太长，请不要超过12个字符！");
			parent.goToolsBtn(false);
			return true;
		}
		var  format = selectPath_File[selectPath_File.length-1].split(".")[1].toLowerCase();
		if ( format !="csv" ) {
	  		mini.alert("对不起，不是.CSV格式！\n\n请您重新上传CSV格式文件！");
	  		parent.goToolsBtn(false);
	     	return true;
		}
		return false;
		 ...
		 //用H5的FormData对象对表单数据进行构造
			...
			$.ajax({
		        ...
		        complete: function(xhr,status){
		            $("#importForm :submit").removeAttr("disabled");
		        },
...
<form method="post" action="${pageContext.request.contextPath}/admin/saveImportOrgUser.action" enctype="multipart/form-data" name="_fmImportFile" id="fmImportFile">
<input type="hidden" name="__Click" value="0"><table border="0" cellpadding="1" cellspacing="2" style="width:98%;margin:0 auto;top:30px">
	<tr>
		<td align="center"> 
	...
	<tr>
		...
		<p style='font-size:14px;color:red'>然后另存为.CSV格式的文件进行上传！</p>
		</td>
	</tr>
</table></form>
</body>
