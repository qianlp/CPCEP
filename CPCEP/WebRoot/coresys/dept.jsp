...
<%
	...
	String pDeptName=request.getParameter("deptName");
	String pDeptId=request.getParameter("deptId");
	pDeptName=new String(pDeptName.getBytes("iso-8859-1"),"utf-8");
	 if("".equals(pDeptName)){
	 	pDeptName = "无";
	 }
	 if("".equals(pDeptId)){
...
<body text="#000000" bgcolor="#FFFFFF">
	<form method="post" action="${pageContext.request.contextPath}/admin/addDept.action" name="addNewDept">
		<input type="hidden" name="dept.nodeLevel" value="${dept.nodeLevel }">
		<script type="text/javascript">
		...
		<table border="0" cellpadding="1" cellspacing="2" style="width:98%;margin:0 auto">
			<tr>
				<td colspan="2" style="display:none">
					部门ID：<input name="dept.uuid" value=""/>
				</td>
			</tr>
			<tr>
				<td align="right" style="width:85px">部门名称：</td>
				<td><input name="dept.deptName" value="" class="mini-textbox"
					style="width:100%;"></td>
			</tr>
			<tr>
				<td align="right">上级部门：</td>
				<td>
					<input name="dept.deptPid" value="<%=pDeptId %>"  text="<%=pDeptName %>"
			...
			<tr>
				...
				<td align="right"><input name="dept.deptUserId"
			...
			<tr>
				...
				<td>
					<label><input type="radio" name="position" value="0" checked style="border-width:0px" onclick="checkPosition(this)">保持当前位置</label>
					<br> 
					<label> <input type="radio" name="position"	value="1" style="border-width:0px"  onclick="checkPosition(this)">在</label> 
					<input name="sameLevelNodeId" value="" class="mini-combobox"
					style="width:150px;" textField="text" valueField="id" emptyText="请选择..." showNullItem="true" nullItemText="请选择..." onbeforeshowpopup="loadSameLevDept" enabled="false"
					id="sameLevelNodeId">之前
				</td>
			</tr>
		</table>
	</form>
