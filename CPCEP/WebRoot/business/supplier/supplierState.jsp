...
<head>
    <title>供应商状态</title>
    <meta name="description" content="供应商状态">
    <meta name="content-type" content="text/html; charset=UTF-8">
    <script src="https://code.jquery.com/jquery-3.3.1.min.js"></script>
    <script type="text/javascript" src="${pageContext.request.contextPath}/js/ht/flexpaper/jquery-1.8.3.js"></script>
...
<div class="mini-layout" style="width:100%;height:100%;" id="layout">
    ...
    <div region="west" style="overflow:hidden;border-bottom:0"
        ...
        <div class="mini-fit" style="padding:2px;border-top:0;border-left:0;border-right:0;">
            <div id='categoryTree' class='mini-tree' style="height:100%;" showTreeIcon='true' showCheckBox='false' checkRecursive='true' expandOnLoad='0' expandOnDblClick='false'
                resultAsTree="false" onnodeclick="nodeclick">
            </div>
        </div>
    </div>
    <%--中心表格--%>
    <div region="center"
        ...
        <div class="mini-toolbar">
            ...
            <a class="mini-button" iconCls="icon-edit" onclick="checkSupplier(2)">审核通过</a>
            <a class="mini-button" iconCls="icon-edit" onclick="checkSupplierFail(8)">审核拒绝</a>
            <a class="mini-button" iconCls="icon-edit" onclick="enable();">禁用</a>
        </div>
...
<script>
  mini.parse();
   ...
   gTree.loadData(treeBut);
    ...
    grid.load({sortField:"registerTime",sortOrder:"desc"});

   /*  var gTree = mini.get("categoryTree");
    var categoryUuid = "-1";//默认pid为-1 */
    
    function findsupplierAudited(status){
    	 grid.setUrl(gDir+'/business/findPage.action?menuId=${menuId}');
    	 grid.load({sortField:"registerTime",sortOrder:"desc",wfStatus:status});
    }
    //树节点点击事件
    var scopeId;
   /*  function nodeclick(e){
        ...
        grid.load({scopeId:categoryUuid, name : $("input[name=name]").val()});
    } */
    function nodeclick(e){
    	var nodeName=e.node.text;
    	if(nodeName =="已审批")findsupplierAudited(2);
    	if(nodeName =="待审核")findsupplierAudited(9);
    	if(nodeName =="审核失败")findsupplierAudited(8);
	   ...
	   mini.open({
		   ...
		   ondestroy:function(data){
			   if(data=="close"){
			    ...
			    $.ajax({
                  url: gDir+"/business/check.action?id="+strDocID + "&checkStatus=" + checkStatus+"&content="+data,
                  success:function(){
                      mini.unmask(document.body);
                      mini.alert("操作成功！");
                      grid.reload();
