        ...
        <div class='mini-fit' style='width:100%;height:100%;'>
            <div id='miniDataGrid' class='mini-datagrid' style="width:100%;height:100%;" idField='uuid' sizeList='[5,10,15,30]' pageSize='15' multiSelect='true' showColumnsMenu='true'>
                <div property='columns'>
                    ...
                    <div field='code' width='120'  headerAlign='center' align='center'>招标编号</div>
					...
					<div field='bidUuid' width='120' headerAlign='center' align='center' renderer='getStatus'>投标状态</div>
                </div>
            </div>
        </div>
<script>
    ...
    grid.setUrl(gDir + '/business/bid/bidSuppliers.action?uuid=${uuid}');//bidFileId
    var pid = "-1";//默认pid为-1
    grid.load({pid:pid});
	...
	*/
	...
	function getBidCount(){
        var rows = grid.findRows(function(row){
            if(row.bidUuid != null && row.bidUuid!="") return true;
        });
        return rows.length;
    }
	function open() {
        var fullDate = "${biddingFile.biddingBulletin.bidOpenTime}".split("-");
        var openTime = new Date(fullDate[0], fullDate[1]-1, fullDate[2].split(" ")[0], 0, 0, 0);
        var now = new Date();
        var title = "确认开标?";
        if(openTime > now)
            title = "未到开标时间，是否提前开标?";
        else if(getBidCount() < 3)
            title = "投标不足三家，是否确认开标?";
		...
		mini.confirm(title,"确认框", function(r){
			if(r !="ok")
			    return ;
			$.ajax({
				url: gDir + '/business/bid/open.action?uuid=${uuid}',//bidFileId
				dataType: "json",
				cache: false,
