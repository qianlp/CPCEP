    ...
    <div region="center"
        ...
        <div class="search_body" id="search_body">
            <div class="search_content">
                <div class="search_title">分类编码：</div>
                <div class="search_field" Operator="@" DataType="text" Item="">
                    <input name="categoryCode" id="categoryCode" class="mini-textbox">
                </div>
            </div>
            <div class="search_content">
                <div class="search_title">分类名称：</div>
                <div class="search_field" Operator="@" DataType="text" Item="">
                    <input name="categoryName" id="categoryName" class="mini-textbox">
                </div>
            </div>
        ...
        <div class='mini-fit' style='width:100%;height:100%;'>
            <div id='miniDataGrid' class='mini-datagrid' style="width:100%;height:100%;" borderStyle="border:0;" idField='unid' sizeList='[5,10,15,30]' pageSize='15' multiSelect='true' showColumnsMenu='true'>
                <div property='columns'>
                    ...
                    <div field='categoryCode' width='120' headerAlign='center' align='center'>设备材料分类编码</div>
                    <div field='categoryName' width='120' headerAlign='center' align='center'>设备材料分类名称</div>
                    <!-- <div name="action" width="100" headerAlign="center">参数项</div> -->
                </div>
            </div>
        </div>
    </div>
...
<script>
    ...
    <%--var menuId = '<%=menuId%>';--%>
     grid.setUrl(gDir+'/business/material/loadCategoriesForPage.action');
 /*     var pid = "-1";//默认pid为-1
    grid.load({pid:pid});  */  
  ...
  /*   grid.on("drawcell",function (e) {
        ...
        if (column.name == "action") {
            ...
            e.cellHtml = '<a href="javascript:openParam(\'' + record.uuid + '\')">点击查看</a>';
        }
    }); */
    ...
    function nodeclick(e){
        pid = e.node.id;
        if(delSelect==e.node.id){
        	gTree.deselect(e.record);
        	delSelect =-1;
        }else{
        	delSelect=e.node.id;
    ...
    //清空查询条件
    function ClearForm(){
        mini.getbyName("categoryCode").setValue("");
        mini.getbyName("categoryName").setValue("");
    }
    /*
    描述：查询
    */
