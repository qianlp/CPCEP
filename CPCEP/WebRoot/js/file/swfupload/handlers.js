	...
	var methods = {
		init : function(options, swfUploadOptions) {
		    ...
		    if(url == null || url == undefined || url == "")
		        url = '/profile/adenexUploadFile.action';
		    if(options.afterComplete != null && options.afterComplete != undefined)
		        handlers.onUploadComplete = options.afterComplete;
		    if(options.onUploadSuccess != null && options.onUploadSuccess != undefined)
		        handlers.onUploadSuccess = options.onUploadSuccess;
            if(options.onDialogOpen != null && options.onDialogOpen != undefined)
                handlers.onDialogOpen = options.onDialogOpen;
		    var config = {
                // Required Settings
                id       		: $(this).attr('id'), // The ID of the DOM object
                uploader 		: gDir + url,  // 上传路径
                // Options
            ...
            return this.each(function() {
				...
				var swfUploadSettings = {
                    file_post_name          : config.fileName,
					//-**********************************************************
					// Button Settings
				...
				if (flashInstalled) {
					...
					btn.render(document.getElementById(settings.id + '-button'));
				//	$button.html('<a class="mini-button" iconCls="icon-'+settings.buttonImage+'" plain="false">' + settings.buttonText + '</a>');
					//$button.html('<a class="mini-button" href="javascript:void(0);"><span class="mini-button-text  mini-button-icon icon-upload" style="padding-top:0px;padding-bottom:0px;">' + settings.buttonText + '</span></a>');
