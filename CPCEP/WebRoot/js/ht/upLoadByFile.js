...
function loadAttachToolbar(id){
	$("#"+id).html("");
	var uploadObj={
		formData       : objAttacheData,
		queueID        : id,				//附件区域标识
		imageZipServer : false   //是否在服务器端进行图片压缩，如果选择文件中是图片的即进行压缩。
	};
			
	$("#"+id).htUpload(uploadObj);
	$("#"+id).css("margin-right","10px");
	$("#"+id).addClass(id);	
	mini.parse();
}
//返回列宽度
...
function goDelAttachFile(s) {
	...
	var arrFile=mini.get("fileGrid").getSelecteds();
    if (arrFile.length > 0) {
        ...
        if (confirm(PublicField.UpMsg5)) {
            ...
            function(i, item) {
                $.ajax({
                    url: "/" + gCommonDB + "/(agtComDelAttach)?OpenAgent&DocID=" + item.unid,
                    dataType: "text",
                    cache: false,
                    success: function(txt) {
                        if (parseInt(txt) == 1) {
