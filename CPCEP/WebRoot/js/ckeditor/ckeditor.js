(function() {
			...
			shrink: function(a, b, c) {
				if (!this.collapsed) {
					...
					d.guard = function(b, d) {
						if (t(b)) return true;
						if (a == CKEDITOR.SHRINK_ELEMENT && b.type == CKEDITOR.NODE_TEXT || d && b.equals(j) || c === false && b.type == CKEDITOR.NODE_ELEMENT && b.isBlockBoundary() || b.type == CKEDITOR.NODE_ELEMENT && b.hasAttribute("contenteditable")) return false;
		...
		CKEDITOR.tools.extend(CKEDITOR.editor.prototype, {
			...
			setKeystroke: function() {
				for (var a = this.keystrokeHandler.keystrokes, b = CKEDITOR.tools.isArray(arguments[0]) ? arguments[0] : [
					[].slice.call(arguments, 0)], c, d, e = b.length; e--;) {
					c = b[e];
					d = 0;
					...
					if (d == "br") y.push(f);
						...
						for (;;) {
							...
							if (!f.isUnknown && !m.isUnknown && !g[d]) if (m.isOptionalClose) u.onTagClose(h);
								...
								m = h
							} else if (d in CKEDITOR.dtd.$listItem && !s(d, h)) u.onTagOpen(d == "li" ? "ul" : "dl", {}, 0, 1);
		...
		CKEDITOR.editable = CKEDITOR.tools.createClass({
			...
			proto: {
				...
				insertElementIntoRange: function(a, b) {
					...
					var g, h;
					if (l) for (;
					(g = b.getCommonAncestor(0, 1)) && (h = CKEDITOR.dtd[g.getName()]) && (!h || !h[d]);) if (g.getName() in CKEDITOR.dtd.span) b.splitElement(g);
	...
	(function() {
		...
		CKEDITOR.on("instanceCreated", function(b) {
			...
			d.on("contentDom", function() {
				...
				CKEDITOR.env.webkit && k.attachListener(b, "keydown", function(a) {
					...
					getNextRange: function(g) {
						...
						if (n && a.length > 1) {
							...
							if (g) for (var j = 0; a[e + j + 1];) {
								for (var o = n.document, g = 0, i = o.getById(d[j].endNode), o = o.getById(d[j + 1].startNode);;) {
	...
	(function() {
		...
		CKEDITOR.on("dialogDefinition", function(a) {
			if (a.editor.plugins.filebrowser) for (var c = a.data.definition, d, b = 0; b < c.contents.length; ++b) if (d = c.contents[b]) j(a.editor, a.data.name, c, d.elements), d.hidden && d.filebrowser && (d.hidden = !h(c, d.id, d.filebrowser))
		})
	})();
